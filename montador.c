/*
Alunos: Theógenes Gabriel Araújo de Amdrade e Daniel Mendonça Paiva
...
//Estrutura para armazenar rótulo
#define MAX_LABELS 100 // Número máximo de rotunos que o montador pode lidar
typedef struct {
    char name[50]; // Nome do rótulo
    int address;   // Endereço (em bytes) do rótulo
} Label;
...
void second_pass(const char *input_filename, const char *output_filename) {
    ...
    while (fgets(line, sizeof(line), input_file) != NULL) {
        ...
        uint32_t instruction_binary = 0; // inicializa
        int instruction_valid = 1; // Flag 
    ...
    fclose(input_file);
    fclose(output_file);
    printf("A Montagem foi completada:  arquivo '%s' gerado.\n", output_filename);
}
...
int main(int argc, char *argv[]) {
    ...
    //Primeira passada: Coleta labels
    first_pass(input_filename);

    //Monta o código e resolve labels
